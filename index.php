...
if(isset($_GET['list'])) {
	...
	foreach ($dircontents as $file) {
		...
		if ($extension == 'mp4') {
			echo '<li><a href=https://example.com/mp4/share/'.str_replace(" ", "%20", $file).' target="_blank">'.str_replace(" ", "%20", $file).'</a></li>';
		}
	}
	echo '</ol>';
	...
	if(isset($_GET['play']) || isset($_GET['playlist'])) {
		...
		foreach($files as $file){
			if($counter == 0) {
				echo '<div id="share"><a href="https://example.com/mp4/share/'.$file.'">'.$file.'</a></div><video id="video" onClick="playPause();" ><source id="source" src="https://example.com/mp4/share/'.$file.'"></video><ul id="playlist"><li class="active"><a href="https://example.com/mp4/share/'.$file.'"></a></li>';
			} else {
				echo'<li><a href="https://example.com/mp4/share/'.$file.'">'.$file.'</a></li>';
			}
			$counter++;
		}
		echo'</ul>';
		...
		echo '<script>window.location.replace("https://example.com/mp4/?playlist");</script>';
	}
...
<?php } else {
	...
	foreach ($dircontents as $file) {
		$extension = pathinfo($file, PATHINFO_EXTENSION);
		if ($extension == 'mp4') {
			echo 'https://example.com/mp4/share/'.str_replace(" ", "%20", $file)."\n";
		}
	}
}?>
